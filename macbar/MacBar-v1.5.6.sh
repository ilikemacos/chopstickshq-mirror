...
echo "-------------------"
if [[ ! -f "$0" ]]; then
  echo "❌ This script must be saved to disk and run directly (e.g. \`bash install-MacBar.sh\`)."
...
EXPECTED_HASH="17183802015c3785fe87fa4b3a96994499a43049d5a5ed09244917ec42db7db3"
...
enum MacBarPlatform {
    static let disablePrivateIOKit: Bool = {
...
enum MacBarMotion {
    ...
    static let bar = Animation.easeInOut(duration: 0.42)
...
struct LivePulseDot: View {
    ...
    var body: some View {
        Circle()
            ...
            .shadow(color: Color.nGreen.opacity(on ? 0.9 : 0.2), radius: on ? 4 : 0)
            .scaleEffect(on ? 1.35 : 1)
            .onAppear {
                guard !MacBarPlatform.disablePrivateIOKit else { return }
...
private let UPDATE_CHECK_URL_FALLBACK = URL(string: "https://chopstickshq.com/macbar/version.json")!
let UPDATE_PAGE_URL  = URL(string: "https://chopstickshq.com/macbar/")!
private let UPDATE_CDN_ORIGIN = "https://chopstickshq.com/macbar"
private let UPDATE_CDN_ORIGIN_LEGACY = "https://chopstickshq.com/macbar"
...
enum UpdateChecker {
        ...
        func tryFetch(_ i: Int) {
            ...
            URLSession.shared.dataTask(with: req) { data, _, _ in
                ...
                let match = items.first { entry in
                    let ver = (entry["version"] as? String ?? entry["title"] as? String ?? "").lowercased()
                    let ch = (entry["channel"] as? String ?? "").lowercased()
                    if !ver.isEmpty {
                        return ver == vLower
                            || ver.replacingOccurrences(of: "v", with: "") == vLower.replacingOccurrences(of: "v", with: "")
                    }
                    return ch == chLower
                ...
                if let highlights = match["highlights"] as? [String], !highlights.isEmpty {
                    let bullets = highlights.prefix(3).map { "• \($0)" }
                    completion(bullets.joined(separator: "\n"))
                    return
                }
    ...
    private static func checkPendingUpdateResult() {
        ...
        DispatchQueue.main.asyncAfter(deadline: .now() + 0.6) {
            ...
            alert.informativeText = detail + "\n\nCheck ~/Library/Logs/rNitro/update.log or download the App ZIP from chopstickshq.com/macbar."
        ...
        fetchVersionInfo { info in
            DispatchQueue.main.async {
                guard let info = info else {
                    ...
                    alert.informativeText = "Could not reach the update server (chopstickshq.com/macbar). Check your internet connection and try again."
                    alert.alertStyle = .warning
                    alert.runModal()
                    return
                }
...
enum UpdateInstaller {
    ...
    private static func showDownloadProgress(for version: String) {
        ...
        let detail = NSTextField(labelWithString: "Fetching \(version) from chopstickshq.com/macbar")
    ...
    private static func performInstall(remoteVersion: String) -> InstallResult {
        ...
        if head.count >= 2, head[0] == 0x3C, head[1] == 0x21 {
            return .failure("Got an HTML error page instead of the App ZIP (missing file on server). Download \(zipName) manually from chopstickshq.com/macbar.")
        }
...
fileprivate final class SMCReader {
    ...
    func smcReadings(preferDie: Bool = true) -> [Double] {
        if MacBarPlatform.disablePrivateIOKit { return [] }
...
fileprivate final class IOReportPowerReader {
    ...
    func sample() -> SocPowerSample? { nil }
...
class CPUMonitor: ObservableObject {
    ...
    static func estimatePackagePowerWatts(usage: Double, baseClock: Double, boostClock: Double,
                                          ...
                                          lowPowerMode: Bool = false, gpuUsage: Double = 0) -> Double {
        let idle = 2.4
        let ceiling = chipPowerCeiling(cpuName)
        let cpuLoad = max(0, min(100, usage)) / 100.0
        let gpuLoad = max(0, min(100, gpuUsage)) / 100.0
        let clockScale = baseClock > 0 ? min(1.22, boostClock / baseClock) : 1.0
        var watts = idle + (ceiling - idle) * pow(cpuLoad, 0.92) * clockScale
        watts += min(18, 22 * gpuLoad)
        switch thermal {
        case .fair: watts *= 1.03
        case .serious: watts *= 1.07
    ...
    static func thermalDisplayValue(_ state: ProcessInfo.ThermalState, usage: Double, gpuUsage: Double = 0) -> Double {
        let cpu = max(0, min(100, usage)) / 100.0
        let gpu = max(0, min(100, gpuUsage)) / 100.0
        let heat = pow(min(1, 0.78 * cpu + 0.22 * gpu), 0.72)
        switch state {
        case .nominal:  return 38 + 52 * heat
        case .fair:     return 52 + 42 * heat
        case .serious:  return 68 + 32 * heat
        case .critical: return 82 + 26 * heat
        @unknown default: return 38 + 52 * heat
        }
    }
    ...
    static func resolveTemperature(state: ProcessInfo.ThermalState, usage: Double, smcReadings: [Double], gpuUsage: Double = 0) -> (temp: Double, source: String) {
        let estimate = thermalDisplayValue(state, usage: usage, gpuUsage: gpuUsage)
    ...
    private func update() {
        ...
        let now = Date()
        let cpu: (avg: Double, perCore: [Double])? = updateCPUUsage()
    ...
    private func applyCPUUsage(_ sample: (avg: Double, perCore: [Double]), includePerCore: Bool) {
        let alpha = MonitorActivity.popoverOpen ? 0.72 : 0.55
        ...
        _ = PublishCoalesce.set(&totalUsage, to: nextUsage, epsilon: 0.04)
    ...
    private func sampleMemory() -> MemorySample? {
        ...
        let freePages = Double(stats.free_count + stats.inactive_count + stats.speculative_count + stats.purgeable_count)
        ...
        _ = sysctlbyname("kern.memorystatus_vm_pressure_level", &pressureLevel, &pressureSize, nil, 0)
        let pressure: String
        if pressureLevel >= 4 || usedPct >= 94 || freeGB < 0.35 { pressure = "Critical" }
        else if pressureLevel >= 2 || usedPct >= 84 || compressedGB > totalGB * 0.22 { pressure = "Warning" }
    ...
    private func applyDerived(_ sample: DerivedSample) {
        ...
        let resolved = CPUMonitor.resolveTemperature(state: sample.state, usage: usage, smcReadings: sample.sensorReadings, gpuUsage: GPUMonitor.shared.usage)
        ...
        let estimate = Self.estimatePackagePowerWatts(
            ...
            gpuUsage: GPUMonitor.shared.usage
        )
        let ceiling = Self.chipPowerCeiling(cpuName) * 1.15
        isLowPowerModeEnabled = sample.lpm
        thermalState = sample.state
        tempSource = MacBarPlatform.disablePrivateIOKit
            ? "Estimated (macOS 26 — SMC disabled)"
            : resolved.source
        smcSensorCount = MacBarPlatform.disablePrivateIOKit ? 0 : sample.sensorReadings.count
...
class DiskActivityMonitor: ObservableObject {
    ...
    private var lastIOSample: Date?

    func syncHistoryBuffer() {
        let cap = MonitorActivity.recordsHistory ? 90 : 0
    ...
    private func sample() {
        guard let totals = Self.readBlockBytes() else { return }
        let now = Date()
        defer {
            lastReadBytes = totals.read
            lastWriteBytes = totals.write
            lastIOSample = now
        }
        guard let prev = lastIOSample else { return }
        let dt = now.timeIntervalSince(prev)
        guard dt > 0.2, dt < 30 else { return }
        let readDelta = totals.read >= lastReadBytes ? totals.read - lastReadBytes : 0
        let writeDelta = totals.write >= lastWriteBytes ? totals.write - lastWriteBytes : 0
        let readMBps = Double(readDelta) / dt / 1_048_576.0
        let writeMBps = Double(writeDelta) / dt / 1_048_576.0
        let activity = readMBps + writeMBps
        DispatchQueue.main.async {
            self.readMBps = readMBps
            self.writeMBps = writeMBps
            if MonitorActivity.recordsHistory {
                self.activityRing.append(activity)
                self.activityHistory = self.activityRing.asArray
            }
        }
    }

    private static func readBlockBytes() -> (read: UInt64, write: UInt64)? {
        var iter: io_iterator_t = 0
        ...
        while service != 0 {
            ...
            if let stats = IORegistryEntryCreateCFProperty(service, "Statistics" as CFString, kCFAllocatorDefault, 0)?.takeRetainedValue() as? [String: Any] {
                func u64(_ keys: [String]) -> UInt64 {
                    for k in keys {
                        if let n = stats[k] as? NSNumber { return n.uint64Value }
                    }
                    return 0
                }
                read += u64(["Bytes (Read)", "Bytes Read", "Total Bytes Read"])
                write += u64(["Bytes (Write)", "Bytes Written", "Total Bytes Written"])
                found = true
            }
            service = IOIteratorNext(iter)
        }
...
final class ProcessMonitor: ObservableObject {
    ...
    func start() {
        ...
        let interval = MonitorActivity.processInterval
...
enum RNitroEasterEgg {
    ...
    static func reveal() {
        ...
        let body = """
        MacBar \(CURRENT_VERSION)
        Channel: \(RNITRO_BUILD_CHANNEL)

        \(pick)

        chopstickshq.com/macbar
        """
        let alert = NSAlert()
    ...
    func log(_ message: String, category: String = "app") {
        ...
        Self.logQueue.async {
            ...
            if !fm.fileExists(atPath: legacy.path) {
                let note = "MacBar logging moved to rnitro.log (Enable Logging)\n"
...
enum MonitorActivity {
    ...
    static var cpuInterval: TimeInterval {
        if DeveloperModeStore.shared.forceHighSampleRate { return 0.5 }
        if popoverOpen || CompileFarmDetector.shared.shouldForceSampling { return 0.5 }
        if PolitePeer.shared.shouldEaseSampling { return idleProfile == .aggressive ? 2.0 : 1.5 }
        return idleProfile == .aggressive ? 1.5 : 1.0
    }

    static var gpuInterval: TimeInterval { popoverOpen ? 1.0 : 2.0 }
    static var networkInterval: TimeInterval { popoverOpen ? 1.0 : 2.0 }
    static var batteryInterval: TimeInterval {
        ...
        if bat.isPresent, !bat.isOnAC, !bat.isCharging {
            if bat.levelPercent < 20 || load >= 60 { return 2.0 }
            if bat.levelPercent < 40 || load >= 40 { return 3.0 }
            return idleProfile == .aggressive ? 5.0 : 3.0
        }
        if bat.isPresent, bat.isCharging { return 3.0 }

        return idleProfile == .aggressive ? 8.0 : 5.0
    }
    static var memoryInterval: TimeInterval {
        if popoverOpen { return 1.0 }
        if PolitePeer.shared.shouldEaseSampling { return 6.0 }
        return idleProfile == .aggressive ? 4.0 : 2.0
    }
    static var diskInterval: TimeInterval {
        if popoverOpen { return 1.0 }
        if PolitePeer.shared.shouldEaseSampling { return 8.0 }
        return 4.0
    }
    ...
    static var tempSampleInterval: TimeInterval {
        if popoverOpen { return 1.0 }
        return idleProfile == .aggressive ? 3.0 : 2.0
    }
    static var processInterval: TimeInterval { popoverOpen ? 1.2 : 3.0 }
    ...
    static var includeSmcSample: Bool {
        if MacBarPlatform.disablePrivateIOKit { return false }
        return popoverOpen
    ...
    static var historyCapacity: Int { popoverOpen ? 90 : 0 }
    ...
    static func setPopoverOpen(_ open: Bool) {
        ...
        if open {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.55) {
                guard MonitorActivity.popoverOpen else { return }
        ...
        while service != 0 {
            ...
            if let stats = IORegistryEntryCreateCFProperty(service, "PerformanceStatistics" as CFString, kCFAllocatorDefault, 0)?.takeRetainedValue() as? [String: Any] {
                ...
                for (key, raw) in stats {
                    ...
                    guard lower.contains("utilization") else { continue }
...
struct MiniGraphView: View {
    ...
    var body: some View {
        GeometryReader { g in
            ...
            let pts: [CGPoint] = history.enumerated().map { i, v in
                let frac = min(max(v, 0), cap) / cap
                return CGPoint(x: Double(i) * step, y: h - frac * h)
            }
            Path { p in
        ...
        .stroke(color.opacity(0.9), style: StrokeStyle(lineWidth: 1.4, lineJoin: .round))
        }
    }
}
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            Text(value)
                ...
                .animation(MacBarMotion.snap, value: value)
                .lineLimit(1)
            ...
            Button(action: {
                ...
                HStack(spacing: 8) {
                    ...
                    Image(systemName: "chevron.down")
                        ...
                        .animation(MacBarMotion.snap, value: isExpanded)
                }
...
struct GraphView: View {
    ...
    var body: some View {
        MiniGraphView(history: history, color: color, maxValue: 100)
    }
}
...
struct PowerGraphView: View {
    ...
    var body: some View {
        MiniGraphView(history: history, color: color, maxValue: maxWatts)
    }
}
    ...
    var body: some View {
        HStack(spacing: 8) {
            ...
            GeometryReader { g in
                ZStack(alignment: .leading) {
                    ...
                    Capsule()
                        ...
                        .frame(width: max(0, g.size.width * core.usage / 100))
                        .animation(MacBarMotion.bar, value: core.usage)
                }
            }.frame(height: 4)
            Text(String(format: "%.0f%%", core.usage))
                ...
                .foregroundColor(.secondary)
                .contentTransition(.numericText())
                .animation(MacBarMotion.snap, value: core.usage)
...
enum ChopsticksAIKB {
    ...
    static let intents: [ChopsticksAIIntent] = [
        ...
        ChopsticksAIIntent(
            ...
            priority: 60,
            answer: "Email mzx+chopsticks@lam.ws.\n\nFor bugs in MacBar specifically, opening a GitHub issue is faster:\nhttps://github.com/ilikemacos/MacBar/issues/new",
        ...
        ChopsticksAIIntent(
            id: "rnitro.battery",
            product: "rnitro",
            label: "What battery info does MacBar show?",
            priority: 55,
            answer: "MacBar shows battery percentage, whether you're on AC or battery, charging state, and a low-battery alert at a threshold you choose.\n\nFor deeper battery work — power draw in watts, per-process energy attribution, and drain explanations — Fathom Air is the dedicated tool, and it's also free.",
        ...
        ChopsticksAIIntent(
            ...
            product: "rnitro",
            label: "What AI providers does MacBar support?",
            priority: 55,
        ...
        ChopsticksAIIntent(
            ...
            priority: 60,
            answer: "Intel Macs are no longer receiving updates. The last Intel build is v1.2.5-Beta, still downloadable from the Intel Mac tab on the MacBar page.\n\nApple Silicon (M1 and newer) gets all current development.",
        ...
        ChopsticksAIIntent(
            ...
            priority: 60,
            answer: "There's a Linux pre-release: v0.1-Linux-Beta-Pre-release-x64.\n\nIt's a GTK 4 + libadwaita desktop app needing Python 3.10+. Download the tarball or install-MacBar-linux.sh from the Linux page and run:\nbash install-MacBar-linux.sh\n\nIt installs to ~/.local/share/macbar. Apple-only sensors (SMC, Low Power Mode) show as N/A.",
        ...
        ChopsticksAIIntent(
            id: "rnitro.safety",
            product: "rnitro",
            label: "Is MacBar safe to run?",
            priority: 55,
            answer: "The installers verify their own SHA-256 checksum before running, and refuse to continue if it doesn't match.\n\nThe full app source is included inline in the shell installers, so you can read everything before executing. MacBar is also on GitHub.\n\nNo telemetry, no account, no data collection.",
        ...
        ChopsticksAIIntent(
            ...
            priority: 60,
            answer: "macOS: quit MacBar from the menu bar, then delete MacBar.app from ~/Applications (or /Applications).\n\nTo remove settings too:\ndefaults delete com.chopsticks.rnitro\n\nLinux: rm -rf ~/.local/share/macbar ~/.local/bin/macbar ~/.local/share/applications/rnitro.desktop\n\nWindows: right-click the tray icon → Exit, then delete the folder you unzipped.",
        ...
        ChopsticksAIIntent(
            ...
            priority: 55,
            answer: "The current macOS build is MacBar v1.5.6.\n\nOther platforms:\n• CLI — v0.2-cli\n• Linux — v0.1 Beta pre-release\n• Windows — v6.0.2 (deprecated)\n\nThe live numbers always come from chopstickshq.com/macbar/version.json.",
        ...
        ChopsticksAIIntent(
            ...
            priority: 55,
            answer: "That's the installer protecting you — it downloaded a file whose SHA-256 doesn't match what version.json publishes, so it refused to install.\n\nUsually it's a truncated or cached download. Try again; if it keeps failing, the mirror you hit may be stale. Report it at github.com/ilikemacos/MacBar/issues/new and include both hashes it printed.",
        ...
        ChopsticksAIIntent(
            ...
            priority: 55,
            answer: "If the SMC sensor keys for your chip aren't recognised, MacBar falls back to macOS's thermal state and interpolates from CPU load — so the number stays plausible but isn't a direct reading.\n\nOn Windows, temperatures need administrator rights; Windows hides thermal sensors from unelevated processes.\n\nOn Linux, Apple-only sensors always show N/A.",
...
struct CSAIAuthSession: Codable, Equatable {
    var accessToken: String
    var refreshToken: String
    var expiresAt: Int
    var user: CSAIAuthUser
...
struct CSAIAuthUser: Codable, Equatable {
    var id: String
    var email: String?
}

@MainActor
final class CSAIAuthStore: ObservableObject {
    static let shared = CSAIAuthStore()
    static let sessionKey = "chopsticksAI.cloud.session"
    static let apiURL = URL(string: "https://chopstickshq.com/api/chopsticks-ai")!

    @Published private(set) var session: CSAIAuthSession?
    ...
    func restore() {
        guard let data = UserDefaults.standard.data(forKey: Self.sessionKey),
              let s = try? JSONDecoder().decode(CSAIAuthSession.self, from: data)
        else { return }
        session = s
    }
    ...
    func signIn() async {
        ...
        guard email.contains("@"), password.count >= 6 else {
            formError = "Use the same email and password as chopsticksAI (min 6 characters)."
            return
        }
        busy = true
        defer { busy = false }
        do {
            let obj = try await api(action: "authSignIn", body: ["email": email, "password": password])
            try apply(obj)
            passwordDraft = ""
            statusMessage = "Signed in as \(self.email)."
        } catch {
    ...
    func signUp() async {
        ...
        let password = passwordDraft
        guard email.contains("@"), password.count >= 6 else {
            formError = "Email and password (min 6) required."
            return
        }
        busy = true
        defer { busy = false }
        do {
            let obj = try await api(action: "authSignUp", body: ["email": email, "password": password])
            if obj["access_token"] is String {
                try apply(obj)
                passwordDraft = ""
            ...
            formError = obj["error"] as? String ?? "Could not create account."
        } catch {
            formError = error.localizedDescription
        }
    }
    ...
    func freshToken() async -> String? {
        ...
        if Double(s.expiresAt) > Date().timeIntervalSince1970 + 60 {
            return s.accessToken
        }
        do {
            let obj = try await api(action: "authRefresh", body: ["refresh_token": s.refreshToken])
            try apply(obj)
    ...
    private func apply(_ obj: [String: Any]) throws {
        ...
        session = CSAIAuthSession(
            accessToken: access,
            refreshToken: refresh,
            expiresAt: expiresAt,
            user: CSAIAuthUser(id: uid, email: userObj["email"] as? String)
        )
        persist()
    }

    private func api(action: String, body: [String: Any]) async throws -> [String: Any] {
        ...
        payload["client"] = "macbar"
        req.httpBody = try JSONSerialization.data(withJSONObject: payload)
        let (data, resp) = try await URLSession.shared.data(for: req)
        let obj = (try? JSONSerialization.jsonObject(with: data)) as? [String: Any] ?? [:]
        guard let http = resp as? HTTPURLResponse, (200..<300).contains(http.statusCode) else {
            throw NSError(domain: "cs.AI", code: (resp as? HTTPURLResponse)?.statusCode ?? -1, userInfo: [
                NSLocalizedDescriptionKey: obj["error"] as? String ?? "Sign-in failed."
            ])
        }
        return obj
    }
...
enum AIProvider: String, CaseIterable, Identifiable {
    ...
    var setupHint: String {
        ...
        case .chopsticks, .chopsticksSuper, .chopsticksCode:
            return "cs.AI is built in. Sign in with your chopsticksAI email and password to use the same account, quota, and Pro as the Mac app. Guest chat still works without an account. ChopCode is the coding plate. Offline falls back to an on-device knowledge base."
        case .lmStudio:
...
struct OpenRouterModelChoice: Identifiable, Hashable {
    let id: String
    let name: String
    var label: String {
...
enum OpenRouterChatModels {
    ...
    static var fallback: [OpenRouterModelChoice] {
        presets.map { OpenRouterModelChoice(id: $0, name: $0) }
    }
}
...
final class AIChatModel: ObservableObject {
    ...
    @Published var openRouterModels: [OpenRouterModelChoice] = OpenRouterChatModels.fallback
    @Published var openRouterModelsLoading = false
    @Published var openRouterModelSearch = ""
    @Published var showOpenRouterModelPicker = false
    ...
    var filteredOpenRouterModels: [OpenRouterModelChoice] {
        let q = openRouterModelSearch.trimmingCharacters(in: .whitespacesAndNewlines).lowercased()
        guard !q.isEmpty else { return openRouterModels }
    ...
    func saveApiKey() {
        ...
        showKeyEditor = false
        apiKeyDraft = ""
        if selectedProvider == .openRouter { refreshOpenRouterModels() }
    ...
    func openRouterModelLabel(_ id: String) -> String {
        openRouterModels.first(where: { $0.id == id })?.label ?? id
    }
    ...
    func refreshOpenRouterModels() {
        ...
        Task {
            let key = self.resolvedKey(for: .openRouter)
            let result = await Self.fetchOpenRouterCatalog(apiKey: key)
            await MainActor.run {
                self.openRouterModelsLoading = false
                if case .success(let list) = result, !list.isEmpty {
                    self.openRouterModels = list
                }
            }
        }
    }
    ...
    func sendMessage() {
        ...
        if provider == .chopsticks || provider == .chopsticksSuper || provider == .chopsticksCode {
            ...
            Task {
                ...
                let token = await CSAIAuthStore.shared.freshToken()
                let reply = await Self.requestChopsticks(messages: history, tier: plate, accessToken: token)
    ...
    nonisolated private static func requestChopsticks(messages: [ChatMessage], tier: String, accessToken: String?) async -> String {
        ...
        req.setValue("MacBar/1.5.6", forHTTPHeaderField: "User-Agent")
    ...
    nonisolated private static func fetchOpenRouterCatalog(apiKey: String?) async -> Result<[OpenRouterModelChoice], Error> {
        var req = URLRequest(url: URL(string: "https://openrouter.ai/api/v1/models")!)
        req.httpMethod = "GET"
        req.setValue("https://chopstickshq.com/macbar/", forHTTPHeaderField: "HTTP-Referer")
        req.setValue("MacBar", forHTTPHeaderField: "X-Title")
        if let key = apiKey, AIKeyUtil.isUsableCloudKey(key) {
            req.setValue("Bearer \(AIKeyUtil.sanitize(key))", forHTTPHeaderField: "Authorization")
        }
        ...
        do {
            ...
            guard let http = resp as? HTTPURLResponse else { throw URLError(.badServerResponse) }
            guard http.statusCode == 200 else {
                return .failure(NSError(domain: "OpenRouter", code: http.statusCode,
                                        userInfo: [NSLocalizedDescriptionKey: "HTTP \(http.statusCode)"]))
            }
            guard let json = try JSONSerialization.jsonObject(with: data) as? [String: Any],
                  let rows = json["data"] as? [[String: Any]] else {
                return .success(OpenRouterChatModels.fallback)
            }
            var models: [OpenRouterModelChoice] = []
            models.reserveCapacity(rows.count)
            for row in rows {
                guard let rawID = row["id"] as? String else { continue }
                let id = rawID.trimmingCharacters(in: .whitespacesAndNewlines)
                guard !id.isEmpty else { continue }
                let rawName = (row["name"] as? String) ?? id
                let name = rawName.trimmingCharacters(in: .whitespacesAndNewlines)
                models.append(OpenRouterModelChoice(id: id, name: name.isEmpty ? id : name))
            }
            models.sort { $0.id.localizedCaseInsensitiveCompare($1.id) == .orderedAscending }
            var seen = Set(models.map(\.id))
            for preset in OpenRouterChatModels.presets.reversed() where !seen.contains(preset) {
                models.insert(OpenRouterModelChoice(id: preset, name: preset), at: 0)
                seen.insert(preset)
            }
...
struct CSAISignInCard: View {
    @Environment(\.uiMetrics) private var metrics
    @ObservedObject private var auth = CSAIAuthStore.shared

    var body: some View {
        VStack(alignment: .leading, spacing: 10) {
            Text("chopsticksAI account")
                .font(rNitroFont(.label, metrics: metrics, weight: .semibold))
            Text("Same email and password as chopsticksAI on the web or Mac app. Guest chat works without signing in.")
                .font(rNitroFont(.caption, metrics: metrics)).foregroundColor(.secondary)
            if auth.isSignedIn {
                Text("Signed in as \(auth.email.isEmpty ? "your account" : auth.email)")
                ...
                SecureField("Password", text: $auth.passwordDraft)
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 8).stroke(Color.border.opacity(0.6), lineWidth: 1))
                HStack(spacing: 10) {
                    MinimalButton(title: auth.busy ? "Working…" : "Sign in", disabled: auth.busy) {
                        Task { await auth.signIn() }
                    }
                    Button("Create account") { Task { await auth.signUp() } }
                        .font(rNitroFont(.caption, metrics: metrics))
                        .buttonStyle(.plain)
                        .disabled(auth.busy)
                }
            }
            if !auth.formError.isEmpty {
                Text(auth.formError)
                    .font(rNitroFont(.caption, metrics: metrics))
                    .foregroundColor(.nRed)
            }
            ...
            Link("Open chopsticksAI", destination: URL(string: "https://chopstickshq.com/chopsticks-ai/")!)
                .font(rNitroFont(.caption, metrics: metrics)).foregroundColor(.accent)
        }
    }
}

struct OpenRouterModelPickerSheet: View {
    @ObservedObject var chat: AIChatModel
    ...
    var body: some View {
        VStack(spacing: 0) {
            HStack(spacing: 10) {
                ...
                Spacer()
                if chat.openRouterModelsLoading {
                    ProgressView().controlSize(.small)
                } else {
                    Text("\(chat.openRouterModels.count) models")
                        .font(.system(size: 11))
                        .foregroundStyle(.secondary)
                }
                Button("Refresh") { chat.refreshOpenRouterModels() }
                    .buttonStyle(.plain)
                Button("Done") { dismiss() }
            ...
            HStack(spacing: 8) {
                Image(systemName: "magnifyingglass")
                    .font(.system(size: 12))
                    .foregroundStyle(.secondary)
                TextField("Search by name or id (provider/model)…", text: $chat.openRouterModelSearch)
                    .textFieldStyle(.plain)
                if !chat.openRouterModelSearch.isEmpty {
                    Button { chat.openRouterModelSearch = "" } label: {
            ...
            .padding(.bottom, 8)
            if chat.filteredOpenRouterModels.isEmpty {
                Spacer()
                Text(chat.openRouterModelsLoading ? "Loading models from OpenRouter…" : "No models match your search.")
                    .font(.system(size: 13))
                    .foregroundStyle(.secondary)
                Spacer()
            } else {
                List(chat.filteredOpenRouterModels) { model in
                    Button {
                        chat.setOpenRouterModel(model.id)
                        dismiss()
                    } label: {
                        HStack(alignment: .center, spacing: 10) {
                            ...
                            Spacer(minLength: 8)
                            if model.id == chat.openRouterModel {
        ...
        .onAppear { chat.refreshOpenRouterModels() }
    }
}
...
struct ChatAPISection: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            ScrollView {
                VStack(alignment: .leading, spacing: 14) {
                    ...
                    if chat.selectedProvider == .chopsticks || chat.selectedProvider == .chopsticksSuper || chat.selectedProvider == .chopsticksCode {
                        CSAISignInCard()
                    } else if chat.selectedProvider == .groq {
                        Text("Groq API key")
                            .font(rNitroFont(.label, metrics: metrics, weight: .semibold))
                        ...
                        HStack(alignment: .center, spacing: 10) {
                            VStack(alignment: .leading, spacing: 4) {
                                Text(chat.openRouterModelLabel(chat.openRouterModel))
                                    .font(rNitroFont(.caption, metrics: metrics))
                                    .lineLimit(2)
                                Text(chat.openRouterModelsLoading
                                     ? "Loading every model from OpenRouter…"
                                     : "\(chat.openRouterModels.count) models available")
                                    .font(rNitroFont(.micro, metrics: metrics))
                                    .foregroundColor(.secondary)
                            }
                            Spacer(minLength: 8)
                            MinimalButton(title: "Browse all…", action: { chat.showOpenRouterModelPicker = true })
                        }
                        .onAppear { chat.refreshOpenRouterModels() }
                    ...
                    Text("Privacy: keys stay on this Mac. cs.AI sign-in uses chopstickshq.com (same account as chopsticksAI). Chat messages are sent only to the provider you pick.")
        ...
        .onAppear {
            ...
            chat.apiKeyDraft = ""
            chat.refreshOpenRouterModels()
        }
        .onChange(of: chat.selectedProvider) { _, _ in chat.apiKeyDraft = "" }
        .sheet(isPresented: $chat.showOpenRouterModelPicker) {
            OpenRouterModelPickerSheet(chat: chat)
        }
    }
}
...
struct SettingsAppearanceSection: View {
    ...
    @AppStorage(MonitorPreferences.uiStyleKey) private var uiStyleRaw = MonitorUIStyle.legacy.rawValue
...
struct AIChatView: View {
    @Environment(\.uiMetrics) private var metrics
    @ObservedObject private var chat = AIChatModel.shared
    @ObservedObject private var csaiAuth = CSAIAuthStore.shared
    ...
    private var header: some View {
        HStack(spacing: 10) {
            ...
            if chat.selectedProvider == .chopsticks || chat.selectedProvider == .chopsticksSuper || chat.selectedProvider == .chopsticksCode {
                let signed = csaiAuth.isSignedIn
                Button(action: {
                    if let onOpenAPISetup { onOpenAPISetup() }
                    else { openAPISetupInMainWindow() }
                }) {
                    Text(signed ? (csaiAuth.email.isEmpty ? "Account" : csaiAuth.email) : "Sign in")
                        .font(.system(size: 10, weight: .semibold))
                        .foregroundStyle(signed ? MacBarCursor.green : MacBarCursor.muted)
                        .lineLimit(1)
                        .padding(.horizontal, 7)
                        .padding(.vertical, 3)
                        .background(Capsule().fill(MacBarCursor.hover))
                ...
                .buttonStyle(.plain)
                .help(signed ? "chopsticksAI account" : "Sign in to chopsticksAI")
            }
            Spacer()
            ...
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(spacing: 6) {
                    ...
                    if chat.selectedProvider == .openRouter {
                        Button {
                            chat.showOpenRouterModelPicker = true
                        } label: {
                            HStack(spacing: 4) {
                                Text(chat.openRouterModelLabel(chat.openRouterModel))
                                    .font(.system(size: 11.5, weight: .medium))
                        ...
                        .buttonStyle(.plain)
                        .fixedSize()
                        .help("OpenRouter model — browse all \(chat.openRouterModels.count) models")
                        .onAppear { chat.refreshOpenRouterModels() }
                    }
        ...
        Button(action: { action?() }) {
            VStack(alignment: .leading, spacing: 6) {
                ...
                Text(String(format: "%.0f%%", usedPercent))
                    .font(rNitroFont(.label, metrics: metrics)).foregroundColor(.secondary)
                    .contentTransition(.numericText())
                    .animation(MacBarMotion.snap, value: usedPercent)
                }
                GeometryReader { g in
                    ZStack(alignment: .leading) {
                        Capsule().fill(Color.border.opacity(0.45))
                        Capsule().fill(Color.accent.opacity(0.75))
                            .frame(width: max(0, g.size.width * usedPercent / 100))
                            .animation(MacBarMotion.bar, value: usedPercent)
                    }
                }.frame(height: 4)
                HStack {
    ...
    private var detailRows: [(String, String)] {
        ...
        case .temperature:
            return [
                ...
                ("SMC Sensors", MacBarPlatform.disablePrivateIOKit
                    ? "Disabled on this macOS"
                    : (monitor.smcSensorCount > 0 ? "\(monitor.smcSensorCount) active" : "None (using estimate)")),
...
final class DisplayPreferencesStore: ObservableObject {
    ...
    private static let enStrings: [String: String] = [
        ...
        "dev.verbose": "Enable Logging → ~/Library/Logs/MacBar",
        ...
        "general.enableLogging.hint": "Write local diagnostics to ~/Library/Logs/MacBar (rnitro.log). Never uploaded.",
        ...
        "general.idleHint": "Efficient (default) samples the menubar every few seconds so MacBar stays out of Activity Monitor. Snappy updates faster when the UI is open. Turn off the Power menubar slot if you still see high CPU.",
        ...
        "openMacBar": "Open MacBar",
        "temp.estimatedBanner": "Temperature estimated on this macOS",
    ...
    private static let zhStrings: [String: String] = [
        ...
        "dev.title": "開發者",
        "dev.uiConfig": "UI 設定",
        "dev.verbose": "啟用日誌 → ~/Library/Logs/MacBar",
        "duel.expand": "展開區域網對決。",
        "duel.host": "主機",
        "duel.join": "加入",
    ...
    private static let esStrings: [String: String] = [
        ...
        "dev.verbose": "Activar registro → ~/Library/Logs/MacBar",
        ...
        "general.enableLogging.hint": "Escribe diagnósticos en ~/Library/Logs/MacBar (solo local).",
    ...
    private static let deStrings: [String: String] = [
        ...
        "dev.verbose": "Protokollierung → ~/Library/Logs/MacBar",
        ...
        "general.enableLogging.hint": "Schreibt Diagnosen nach ~/Library/Logs/MacBar (nur lokal).",
...
enum FirstLaunchTips {
    ...
    static func revertToLegacyMonitorIfNeeded() {
        let flag = "rnitro.uiStyle.useLegacyDefault.152"
        let d = UserDefaults.standard
        guard !d.bool(forKey: flag) else { return }
        d.set(MonitorUIStyle.legacy.rawValue, forKey: MonitorPreferences.uiStyleKey)
...
enum CLIIntegration {
    ...
    static let installCommand = "curl -fsSL https://getrnitro.netlify.app/MacBar-CLI.tar.gz -o /tmp/rnitro-cli.tar.gz && mkdir -p /tmp/rnitro-cli && tar xzf /tmp/rnitro-cli.tar.gz -C /tmp/rnitro-cli && bash /tmp/rnitro-cli/install-cli.sh"
...
enum MenuBarStatusFormatter {
    static func slotLabel(_ slot: MenuBarSlot) -> String {
        ...
        case .temp:
            let t = "\(Int(cpu.temperature.rounded()))°"
            return MacBarPlatform.disablePrivateIOKit ? "\(t)~" : t
...
struct AppTabSidebar: View {
    ...
    private func tabButton(_ t: AppTab) -> some View {
        Button(action: { withAnimation(MacBarMotion.snap) { tab = t } }) {
            HStack(spacing: 8) {
            ...
            .animation(MacBarMotion.snap, value: tab)
        }
        .buttonStyle(SoftPressStyle())
    }
}
...
struct EstimatedTempBanner: View {
    @Environment(\.uiMetrics) private var metrics
    var body: some View {
        Text(DisplayPreferencesStore.shared.tr("temp.estimatedBanner"))
            .font(rNitroFont(.caption, metrics: metrics))
            .foregroundColor(.secondary)
            .frame(maxWidth: .infinity, alignment: .leading)
            .padding(.horizontal, 14)
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 0) {
                ...
                if MacBarPlatform.disablePrivateIOKit {
                    EstimatedTempBanner()
                }
...
struct ContentView: View {
    ...
    var body: some View {
        MetricsReader(layout: layout) { _ in
            ...
            .animation(MacBarMotion.bar, value: lowBattery.isLowPowerDim)
            ...
            .onReceive(NotificationCenter.default.publisher(for: .rNitroOpenMainWindow)) { note in
                ...
                if let raw = note.userInfo?["tab"] as? String, let t = AppTab(rawValue: raw) {
                    withAnimation(MacBarMotion.snap) { tab = t }
                }
                NSApp.activate(ignoringOtherApps: true)
    ...
    private var rootContent: some View {
        ...
        .clipShape(RoundedRectangle(cornerRadius: (layout == .popover && !MacBarPlatform.disablePrivateIOKit) ? MacBarMotion.panelCorner : 0, style: .continuous))
    }
...
class AppDelegate: NSObject, NSApplicationDelegate, UNUserNotificationCenterDelegate {
    ...
    func applicationDidFinishLaunching(_ notification: Notification) {
        FirstLaunchTips.revertToLegacyMonitorIfNeeded()
    ...
    @objc private func togglePopover() {
        ...
        if event.type == .rightMouseUp {
            ...
            menu.addItem(withTitle: "Open MacBar", action: #selector(openMainWindowFromMenu), keyEquivalent: "")
    ...
    private func ensureMenuPanel() {
        ...
        hosting.view.layer?.cornerRadius = MacBarMotion.panelCorner
        ...
        let placeholder = NSView(frame: NSRect(origin: .zero, size: popoverSize))
        placeholder.wantsLayer = true
        placeholder.layer?.backgroundColor = NSColor.windowBackgroundColor.withAlphaComponent(0.92).cgColor
        placeholder.layer?.cornerRadius = MacBarMotion.panelCorner
    ...
    private func showMenuPanel(from button: NSStatusBarButton) {
        ...
        let buttonRect = button.window.map { $0.convertToScreen(button.convert(button.bounds, to: nil)) } ?? .zero
        let size = popoverSize
        var origin = NSPoint(x: buttonRect.midX - size.width / 2, y: buttonRect.minY - size.height - 8)
        if let screen = button.window?.screen ?? NSScreen.main {
            let vis = screen.visibleFrame
            origin.x = min(max(origin.x, vis.minX + 8), vis.maxX - size.width - 8)
            if origin.y < vis.minY {
                origin.y = buttonRect.maxY + 8
            }
        }
        panel.setFrame(NSRect(origin: origin, size: size), display: false)
        panel.alphaValue = 1
        panel.orderFrontRegardless()
        if panel.contentViewController == nil, let hosting = popoverHosting {
            DispatchQueue.main.asyncAfter(deadline: .now() + 0.08) {
                guard self.menuPanel?.isVisible == true else { return }
                self.menuPanel?.contentViewController = hosting
                self.applyMenuPanelRounding()
            }
        } else {
            applyMenuPanelRounding()
        }
        MonitorActivity.setPopoverOpen(true)
        installClickAway()
    }

    private func applyMenuPanelRounding() {
        let r = MacBarMotion.panelCorner
        let views = [menuPanel?.contentView, popoverHosting?.view].compactMap { $0 }
        for view in views {
    ...
    private func installClickAway() {
        ...
        clickAwayMonitor = NSEvent.addGlobalMonitorForEvents(matching: [.leftMouseDown, .rightMouseDown]) { [weak self] event in
            ...
            if let button = self.statusItem?.button, let win = button.window {
                ...
                if r.contains(loc) { return }
            }
            DispatchQueue.main.async { self.hideMenuPanel() }
        }
    }
    ...
    @objc private func openMainWindowFromMenu() {
        hideMenuPanel()
        MainWindowController.shared.show(userInfo: ["tab": AppTab.monitor.rawValue])
    }
...
<dict>
    ...
    <key>CFBundleVersion</key><string>1.5.6</string>
