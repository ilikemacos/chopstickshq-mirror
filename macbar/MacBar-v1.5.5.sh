...
EXPECTED_HASH="cd3416565056fc77cc16def26032a2fa7f24f1945048538c28b2699eb91f041a"
...
enum UpdateInstaller {
    ...
    private static func findAppBundle(in dir: URL) -> URL? {
        ...
        for case let url as URL in e {
            ...
            if url.lastPathComponent == "MacBar.app" || url.lastPathComponent == "MacBar.app",
...
final class AIChatModel: ObservableObject {
    ...
    func refreshOpenRouterModels() {
        guard let key = resolvedKey(for: .openRouter), AIKeyUtil.isUsableCloudKey(key) else {
            openRouterModels = OpenRouterChatModels.fallback
            return
        }
    ...
    nonisolated private static func requestChopsticks(messages: [ChatMessage], tier: String, accessToken: String?) async -> String {
        ...
        req.setValue("MacBar/1.5.5", forHTTPHeaderField: "User-Agent")
                    ...
                    if chat.selectedProvider == .chopsticks || chat.selectedProvider == .chopsticksSuper || chat.selectedProvider == .chopsticksCode {
                        ...
                        Picker("OpenRouter model", selection: Binding(
                            ...
                            if chat.openRouterModelsLoading {
                                Text("Loading models…").tag(chat.openRouterModel)
                            }
                            ForEach(chat.openRouterModels) { model in
...
<dict>
    ...
    <key>CFBundleVersion</key><string>1.5.5</string>
