#!/usr/bin/env bash
set -euo pipefail
INSTALL_ROOT="${HOME}/.local/share/macbar"
VENV="${INSTALL_ROOT}/venv"
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
DESKTOP_DIR="${HOME}/.local/share/applications"
AUTOSTART_DIR="${HOME}/.config/autostart"
echo "=== MacBar Linux installer ==="
need() {
...
if [[ ! -d "${SCRIPT_DIR}/linux/macbar" ]]; then
  echo "Missing package at ${SCRIPT_DIR}/linux/macbar"
  exit 1
fi
...
pip install -q psutil || echo "Note: psutil not installed — MacBar will use /proc fallback."
SITE_PACKAGES="${VENV}/lib/python${PYVER}/site-packages"
mkdir -p "${SITE_PACKAGES}"
rm -rf "${SITE_PACKAGES}/macbar"
cp -R "${INSTALL_ROOT}/linux/macbar" "${SITE_PACKAGES}/macbar"
mkdir -p "${DESKTOP_DIR}"
