# install-MacBar-windows.ps1
# MacBar for Windows — lightweight system tray monitor
...
#
# v6.1.1a — Fixed update prompt to always open chopstickshq.com/macbar after OK.
#
# v6.0.1b — Added update checker on launch: compares against version.json on
# chopstickshq.com/macbar (uses the "windows" field) and opens the site to download
# when a newer version is available (e.g. v2 → v6.0.1b).
#
# v6.0.1b — Pre-built MacBar-v6.0.1b.exe available on the website.
...
#   Then: powershell -ExecutionPolicy Bypass -File install-MacBar-windows.ps1

Set-StrictMode -Version Latest
$ErrorActionPreference = "Stop"

$VERSION = "4.2.1-Windows-Final-x86"
$INSTALL_DIR = "$env:LOCALAPPDATA\MacBar"
$EXE_PATH    = "$INSTALL_DIR\MacBar.exe"
$SRC_PATH    = "$INSTALL_DIR\MacBar.cs"
...
Write-Host ""
Write-Host "  MacBar for Windows v$VERSION" -ForegroundColor Cyan
...
static class UpdateChecker {
    ...
    const string VersionUrl     = "https://chopstickshq.com/macbar/version.json";
    const string DownloadPage   = "https://chopstickshq.com/macbar";
    ...
    public static async Task CheckOnLaunchAsync() {
        try {
            ...
            http.DefaultRequestHeaders.Add("User-Agent", "MacBar-Windows/" + CurrentVersion);
            ...
            MessageBox.Show(
                $"You're running {CurrentVersion}. The latest version is {latest}.\n\nOpening chopstickshq.com/macbar so you can update.",
                "MacBar Update Available",
...
static class BtcMonitor {
    ...
    public static async Task FetchLoop(CancellationToken ct) {
        _http.DefaultRequestHeaders.Add("User-Agent", "MacBar-Windows/4.2.1-Windows-Final-x86");
...
class RNitroContext : ApplicationContext {
    ...
    public RNitroContext() {
        ...
        _tray = new NotifyIcon { Visible = true, Text = "MacBar", Icon = MakeIcon(0) };
        _tray.MouseClick += OnTrayClick;

        var menu = new ContextMenuStrip();
        menu.Items.Add("MacBar v4.2.1-Windows-Final-x86").Enabled = false;
...
class MainPopupForm : Form {
    ...
    void BuildMonitorPanel() {
        ...
        var title = UiHelper.MakeLabel("MacBar", 15, FontStyle.Bold, Theme.Text);
    ...
    void ShowDetail(string kind) {
        ...
        MessageBox.Show(string.Join("\n", lines), "MacBar Details", MessageBoxButtons.OK, MessageBoxIcon.Information);
    }
}
...
# ── Compile ───────────────────────────────────────────────────────────────────
Write-Host "Compiling MacBar.exe (this takes a few seconds)..." -ForegroundColor Yellow
...
$addStartup = Read-Host "Add MacBar to startup? (y/n)"
if ($addStartup -eq "y") {
    $regPath = "HKCU:\Software\Microsoft\Windows\CurrentVersion\Run"
    Set-ItemProperty -Path $regPath -Name "MacBar" -Value "`"$EXE_PATH`""
    Write-Host "  Added to startup." -ForegroundColor Green
}

# ── Launch ────────────────────────────────────────────────────────────────────
Write-Host ""
Write-Host "Launching MacBar..." -ForegroundColor Cyan
Start-Process $EXE_PATH

Write-Host ""
Write-Host "MacBar is running in your system tray." -ForegroundColor Green
