...
EXPECTED_HASH="82b571f620f30c1cbf98d9a8cdb70453a8673f259dd7131dfa26afa4cc21dc75"
...
let CURRENT_VERSION = "v1.2.8-Final"
...
enum UpdateInstaller {
    ...
    private static func performInstall(remoteVersion: String) -> InstallResult {
        ...
        guard let staged = findAppBundle(in: extractDir) else {
            ...
            return .failure("MacBar.app not found inside \(zipName). Download manually from getrnitro.netlify.app.")
        }
    ...
    private static func replaceApp(stagedApp: URL, destination: URL) -> ReplaceResult {
        ...
        if isSystemApplicationsBundle(destination.path) {
            ...
            if proc.terminationStatus != 0 {
                ...
                return ReplaceResult(error: "Could not replace MacBar in /Applications. Enter your Mac password when prompted, or download the App ZIP from getrnitro.netlify.app.")
            }
...
class CPUMonitor: ObservableObject {
    ...
    static func resolveTemperature(state: ProcessInfo.ThermalState, usage: Double, smcReadings: [Double]) -> (temp: Double, source: String) {
        ...
        let sensor = sorted[sorted.count / 2]
        ...
        ScrollView {
            VStack(alignment: .leading, spacing: 14) {
                ...
                Toggle(isOn: $dev.verboseLogging) {
                    Text("Verbose logging to ~/Library/Logs/MacBar").font(rNitroFont(.label, metrics: metrics))
                }
                .toggleStyle(.switch)
...
final class DisplayPreferencesStore: ObservableObject {
    ...
    private static let enStrings: [String: String] = [
        ...
        "appearance.title": "Display", "appearance.subtitle": "UI font (search & favorites), size, language, and monitor layout.",
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            VStack(alignment: .leading, spacing: 12) {
                tipRow("1", "Find the menubar icon", "MacBar lives in the top-right menu bar. Click it anytime for live CPU, temp, and per-core stats.")
                tipRow("2", "First launch on macOS", "If Gatekeeper blocks the app: right-click MacBar.app → Open → Open once. No admin password needed for the App ZIP.")
                tipRow("3", "Customize anytime", "Settings → Menubar / Appearance for density, slots, accents. Developer Mode is optional (Beta).")
                tipRow("4", "Recommended install", "App ZIP from getrnitro.netlify.app or chopstickshq.com/macbar — or the Terminal one-liner to skip most Gatekeeper prompts.")
            }
            Button(action: {
...
<dict>
    ...
    <key>CFBundleVersion</key><string>v1.2.8-Final</string>
    <key>CFBundleShortVersionString</key><string>v1.2.8-Final</string>
