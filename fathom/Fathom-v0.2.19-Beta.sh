#!/bin/bash
# Fathom installer v0.2.19-Beta — 4 battery sources
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="ec8fbb3265d548e6d674e19797ebccf2e10f05c8ba479059c2992738aa1914ee"
...
let FATHOM_VERSION = "v0.2.19-Beta"
...
final class PowerStore: ObservableObject {
    ...
    private func applyBattery(_ s: BatSnap) {
        ...
        isOnAC = s.onAC
        batterySourcesUsed = s.sourcesLabel.isEmpty ? "—" : s.sourcesLabel
        minutesIsLiveEstimate = s.minutesIsLiveEstimate
        timeEstimateSource = s.minutesIsLiveEstimate ? "From current draw" : (s.minutes != nil ? "System" : "—")
        ...
        if s.charging {
            ...
            if let m = remainingMinutes {
                ...
                timeText = s.minutesIsLiveEstimate ? "\(base) · draw est." : base
            } else {
    ...
    /// Four independent battery sources, merged for accuracy.
    /// 1) IOPS  2) AppleSmartBattery  3) pmset  4) ioreg
    static func readBattery() -> BatSnap {
        let readings: [BatSourceReading] = [
            readBatteryIOPS(),
