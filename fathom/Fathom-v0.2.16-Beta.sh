#!/bin/bash
# Fathom installer v0.2.16-Beta — rNitro-style dashboard
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="1afb51adefd7b16cdce85523b29f981a6bb22322befd94cc3e7e7686b65cb648"
...
let FATHOM_VERSION = "v0.2.16-Beta"
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if showMore {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            .labelsHidden()
                            Text("% + Time = charge level and how long left. % + W = level and package watts.")
                                .font(uiFont(10))
                                .foregroundColor(.labelTertiary)
                                .fixedSize(horizontal: false, vertical: true)
    ...
    private var powerSection: some View {
        MonitorSection(
            ...
            if power.isCharging {
                ...
                MonitorRow(label: "Draw", value: String(format: "%.1f W", power.chargeWatts), valueColor: .nOrange)
            }
            MonitorRow(label: "Time", value: power.timeText, valueColor: .labelSecondary)
            if prefs.showHealth {
                if let h = power.healthPercent {
...
<key>CFBundleVersion</key><string>0.2.16</string>
