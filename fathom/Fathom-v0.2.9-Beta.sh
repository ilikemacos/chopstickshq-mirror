#!/bin/bash
# Fathom installer v0.2.9-Beta — BatFi menu + dashboard window
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="d2dd1da2b6fbe190855371d07b72660d058e7af4b1ccbf5345690dc48d146725"
...
let FATHOM_VERSION = "v0.2.9-Beta"
...
/// Compact label | value row (screenshot / BatFi style)
...
struct MainRoot: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if showMore {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            Text("⌘-drag the menu bar icon to move it. Slider: near clock → left.")
                                .font(uiFont(11))
                                .foregroundColor(.labelTertiary)
                                .fixedSize(horizontal: false, vertical: true)
            ...
            // ── Why drain? pinned bottom (Fathom signature) ──
            MenuHairline()
...
// MARK: - Full dashboard window (non-menubar UI — matches product mock)

/// Floating dashboard window: large %, KV rows, FATHOM badge — like the marketing card.
    ...
    var body: some View {
        ScrollView(showsIndicators: false) {
            VStack(alignment: .leading, spacing: 0) {
                ...
                HStack(alignment: .top, spacing: 16) {
                    VStack(alignment: .leading, spacing: 6) {
                        Text(power.batteryPresent ? "\(power.levelPercent)%" : "AC")
                            ...
                            .monospacedDigit()
                            .padding(.bottom, 2)
                        Text(FathomStatusCopy.onBatteryLine(power: power))
                            .font(uiFont(13, weight: .regular))
                            .foregroundColor(.labelSecondary)
                        Text(FathomStatusCopy.timeDetail(power: power))
                            .font(uiFont(12, weight: .regular))
                ...
                // Primary KV rows (screenshot layout)
                VStack(spacing: 0) {
                    StatusKVRow(label: "Power Source", value: FathomStatusCopy.powerSourceLine(power: power))
                    StatusKVRow(label: "Energy Mode", value: FathomStatusCopy.energyModeLine(lowPower: lowPower))
                    StatusKVRow(label: "Apps Using Energy", value: topApp)
                    StatusKVRow(
                        label: "Why drain?",
                        value: why.dropPercent > 0
                            ? "−\(why.dropPercent)% · \(why.windowLabel)"
                            : why.windowLabel.isEmpty ? "Last hour" : why.windowLabel
                    )
                }
                .padding(.horizontal, 28)
                .padding(.top, 8)
                ...
                VStack(alignment: .leading, spacing: 8) {
                    ...
                    if energy.top.isEmpty {
                        ...
                        ForEach(energy.top.prefix(8)) { p in
                            HStack {
                                Text(p.name)
                                    .font(uiFont(13, weight: .regular))
                                    .foregroundColor(.labelPrimary)
                ...
                // Expandable power details
                Button {
                    withAnimation(.easeInOut(duration: 0.15)) { showDetails.toggle() }
                } label: {
                    HStack {
                        Text(showDetails ? "Hide power details" : "Show power details")
                            .font(uiFont(12, weight: .medium))
                            .foregroundColor(.labelSecondary)
                        Spacer()
                    ...
                    .padding(.horizontal, 28)
                    .padding(.bottom, showDetails ? 8 : 20)
                }
                .buttonStyle(.plain)

                if showDetails {
                    VStack(spacing: 0) {
                        MonitorRow(label: "Package", value: String(format: "%.1f W", power.packageWatts))
                        if power.isCharging {
                            MonitorRow(label: "Charge in", value: String(format: "%.1f W", power.chargeWatts))
                        } else if power.chargeWatts > 0.05 {
                            MonitorRow(label: "Draw", value: String(format: "%.1f W", power.chargeWatts))
                        }
                        if let h = power.healthPercent {
                            MonitorRow(label: "Health", value: "\(h)%")
                        }
                        if let c = power.cycleCount {
                            MonitorRow(label: "Cycles", value: "\(c)")
                        }
                        ...
                        HStack(spacing: 8) {
                            ...
                            MinimalButton(title: "Check Updates") { UpdateChecker.checkManually() }
                            MinimalButton(title: "Website") { NSWorkspace.shared.open(UPDATE_PAGE_URL) }
                        }
                        .padding(.top, 8)
                        ...
                        Slider(value: $retentionDays, in: 1...30, step: 1)
                            ...
                            .onChange(of: retentionDays) { _, v in DrainLog.shared.setRetention(Int(v)) }
                    }
                    .padding(.horizontal, 28)
                    .padding(.bottom, 24)
                }
        ...
        .frame(minWidth: 420, idealWidth: 440, minHeight: 520, idealHeight: 620)
        .background(Color.bg)
        .preferredColorScheme(.dark)
        .onAppear {
            lowPower = ProcessInfo.processInfo.isLowPowerModeEnabled
...
enum MenuBarBatteryIcon {
    /// Single template image: battery glyph + % (or “AC”).
    /// Drawing both into one image avoids NSStatusItem title clipping under notch / crowding.
    static func image(level: Int, charging: Bool, onAC: Bool, present: Bool) -> NSImage? {
        let label = present ? "\(level)%" : "AC"
        ...
        let gap: CGFloat = 4
        let padX: CGFloat = 2
        // Cap symbol footprint so SF “size” inflation does not over-wide the item
        ...
        let img = NSImage(size: NSSize(width: canvasW, height: canvasH), flipped: false) { rect in
            ...
            // Flatten any hierarchical grays to solid black for template tinting
            NSColor.black.set()
    ...
    /// No separate title — everything is in the composite image.
    static func title(level: Int, present: Bool, onAC: Bool, charging: Bool) -> String {
        _ = (level, present, onAC, charging)
        return ""
    }

    /// Status-item width from composite image (with a little breathing room).
    static func preferredLength(title: String, image: NSImage?) -> CGFloat {
        _ = title
        let w = (image?.size.width ?? 40) + 8
        // Upper bound keeps room for "100%" + bolt; lower bound for "0%"
        return min(max(w.rounded(.up), 36), 100)
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private let popoverSize = NSSize(width: 292, height: 480)
    ...
    @objc func openDashboard() {
        ...
        win.backgroundColor = NSColor(calibratedRed: 0.11, green: 0.11, blue: 0.12, alpha: 1)
        win.appearance = NSAppearance(named: .darkAqua)
        win.contentViewController = hosting
        win.setContentSize(NSSize(width: 440, height: 620))
