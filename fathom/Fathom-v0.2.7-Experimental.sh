#!/bin/bash
# Fathom installer v0.2.7-Beta — IOReport package watts + sleep/wake drain events
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="a4a86af2be01bcf81bfe49e610657212c6dd1f745a7f319bb57d48b14f2ea97a"
...
let FATHOM_VERSION = "v0.2.7-Beta"
...
enum MenuBarBatteryIcon {
    static func image(level: Int, charging: Bool, onAC: Bool, present: Bool) -> NSImage? {
        ...
        // Slightly smaller symbol so title % is less likely to clip on the notch
    ...
    /// Compact title — no leading space (button uses imageLeading + hug)
    static func title(level: Int, present: Bool, onAC: Bool, charging: Bool) -> String {
        if !present { return "AC" }
        return "\(level)%"
    }

    /// Explicit status-item width so the label is never truncated in the menu bar.
    static func preferredLength(title: String, image: NSImage?) -> CGFloat {
        let font = NSFont.monospacedDigitSystemFont(ofSize: 12, weight: .medium)
        let textW = (title as NSString).size(withAttributes: [.font: font]).width
        let imgW = max(image?.size.width ?? 16, 16)
        // image + gap + text + horizontal padding
        let raw = imgW + 4 + textW + 16
        return min(max(raw.rounded(.up), 54), 88)
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private func updateStatusTitle() {
        ...
        let title = MenuBarBatteryIcon.title(
            level: p.levelPercent,
            present: p.batteryPresent,
            onAC: p.isOnAC,
            charging: p.isCharging
        ...
        // Critical: pin length so notch / crowding never clips "100%"
        item.length = MenuBarBatteryIcon.preferredLength(title: title, image: img)
