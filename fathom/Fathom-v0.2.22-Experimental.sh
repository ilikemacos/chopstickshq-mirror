#!/bin/bash
# Fathom installer v0.2.22-Beta — 4 battery sources
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="cdc02db6b95df25aaf216c3e0bba0dcf76e86783ac05590e9f9386987bbc328c"
...
let FATHOM_VERSION = "v0.2.22-Beta"
            ...
            HStack(alignment: .center, spacing: 14) {
                ...
                VStack(alignment: .leading, spacing: 4) {
                    Text(power.batteryPresent ? "\(power.levelPercent)%" : "AC")
                        .font(uiFont(32, weight: .semibold))
                        .foregroundColor(.labelPrimary)
                        .monospacedDigit()
                ...
                Spacer(minLength: 8)
                // Easy escape — close menubar panel
                Button {
            ...
            ScrollView(showsIndicators: false) {
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    MenuSectionHeader(title: FathomCopy.whatsUsingPower)
                        .padding(.horizontal, 18)
                    Text("Estimated from package power × CPU share")
                        .font(uiFont(10, weight: .regular))
                        .foregroundColor(.labelTertiary)
                        .padding(.horizontal, 18)
                        .padding(.bottom, 6)
...
enum MenuBarBatteryIcon {
    ...
    /// Battery glyph + time remaining. Colored (non-template) when charging / LPM / critical.
    static func image(
        level: Int,
        charging: Bool,
        onAC: Bool,
        ...
        let colored = tint != .normal
        let ink: NSColor = colored ? BatteryMenuChrome.tintNSColor(tint) : .black
        ...
        guard let symbolRaw = NSImage(systemSymbolName: name, accessibilityDescription: "Battery")?
            ...
            return textOnlyImage(label, ink: ink, template: !colored)
        }
        ...
        // Template only for normal (system light/dark). Colored states keep true color.
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    /// Roomier panel — less cramped than 300×500
    private let popoverSize = NSSize(width: 340, height: 560)
...
SWIFTEOF
echo "🔨 Compiling…"
swiftc "$WORK_DIR/main.swift" -o "$WORK_DIR/Fathom" -framework SwiftUI -framework Cocoa -framework IOKit -framework ServiceManagement -framework UserNotifications -lIOReport -parse-as-library -O
