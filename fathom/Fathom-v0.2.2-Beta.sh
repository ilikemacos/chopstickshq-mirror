#!/bin/bash
# Fathom installer v0.2.2-Beta — IOReport package watts + sleep/wake drain events
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="9da2a2e8411f8a4b4d7742f464a60740c8188cf514c2112db2f2579c675b9c09"
...
let FATHOM_VERSION = "v0.2.2-Beta"
...
extension Color {
    ...
    static let bg = adaptive(light: rgb(0.97, 0.97, 0.97), dark: rgb(0.04, 0.04, 0.04))
    static let card = adaptive(light: rgb(1.0, 1.0, 1.0), dark: rgb(0.09, 0.09, 0.09))
    static let border = adaptive(light: rgb(0.78, 0.78, 0.78), dark: rgb(0.22, 0.22, 0.22))
    static let accent = adaptive(light: rgb(0.08, 0.08, 0.08), dark: rgb(0.94, 0.94, 0.94))
    static let nGreen = adaptive(light: rgb(0.20, 0.20, 0.20), dark: rgb(0.88, 0.88, 0.88))
    static let nOrange = adaptive(light: rgb(0.35, 0.35, 0.35), dark: rgb(0.72, 0.72, 0.72))
    static let nRed = adaptive(light: rgb(0.12, 0.12, 0.12), dark: rgb(0.95, 0.95, 0.95))
    static let nPurple = adaptive(light: rgb(0.28, 0.28, 0.28), dark: rgb(0.80, 0.80, 0.80))
}

func uiFont(_ size: CGFloat, weight: Font.Weight = .regular) -> Font {
    // Prefer Varela Round when installed (same as rNitro); else system
    if NSFont(name: "Varela Round", size: size) != nil {
        return .custom("Varela Round", size: size).weight(weight)
    }
...
// MARK: - Shared UI components (rNitro-style)
    ...
    var body: some View {
        Button(action: action) {
            Text(title)
                ...
                .padding(.vertical, 6)
                .background(RoundedRectangle(cornerRadius: 6).stroke(Color.accent.opacity(0.45), lineWidth: 1))
        }
        .buttonStyle(.plain)
    }
}
...
struct MonitorSection<Content: View>: View {
    ...
    init(title: String, accent: Color, summary: String, sparkline: [Double]? = nil,
         storageKey: String, defaultExpanded: Bool = true, @ViewBuilder content: @escaping () -> Content) {
        self.title = title
        self.accent = accent
...
struct PowerSection: View {
    ...
    var body: some View {
        ...
        MonitorSection(title: "POWER", accent: .accent, summary: summary,
            ...
            MonitorRow(label: "Package", value: String(format: "%.1f W", power.packageWatts), valueColor: .nOrange)
            MonitorRow(label: "Power source",
                       value: power.packageSource == "sensor" ? "IOReport sensor" : "load estimate",
                       valueColor: power.packageSource == "sensor" ? .nGreen : .secondary)
            if power.isCharging {
                MonitorRow(label: "Charge in", value: String(format: "%.1f W", power.chargeWatts), valueColor: .nGreen)
            } else if power.chargeWatts > 0 {
                MonitorRow(label: power.batteryPresent && !power.isOnAC ? "Battery draw" : "Draw",
                           value: String(format: "%.1f W", power.chargeWatts), valueColor: .nRed)
            }
            if let h = power.healthPercent {
                MonitorRow(label: "Health", value: "\(h)%", valueColor: .accent)
            }
...
struct EnergySection: View {
    ...
    var body: some View {
        ...
        MonitorSection(title: "ENERGY", accent: .nOrange, summary: topName,
                       storageKey: "fathom.section.energy") {
            ...
            ForEach(store.top.prefix(8)) { p in
                VStack(alignment: .leading, spacing: 4) {
                    ...
                    .buttonStyle(.plain)
                    if expanded == p.id {
                        Sparkline(values: p.samples, color: .accent)
                            .frame(height: 28)
...
struct WhySection: View {
    ...
    var body: some View {
        ...
        MonitorSection(title: "WHY DRAIN?", accent: .nPurple, summary: summary,
                       storageKey: "fathom.section.why") {
            Text(why.summary)
                .font(uiFont(12))
                .foregroundColor(.primary)
                .fixedSize(horizontal: false, vertical: true)
            if why.dropPercent > 0 {
                MonitorRow(label: "Drop", value: "~\(why.dropPercent)% · \(why.windowLabel)", valueColor: .nOrange)
            }
            ForEach(Array(why.culprits.enumerated()), id: \.offset) { _, c in
                MonitorRow(label: c.name, value: c.note, valueColor: .accent)
            }
...
struct SettingsSection: View {
    ...
    var body: some View {
        MonitorSection(title: "SETTINGS", accent: .secondary, summary: FATHOM_VERSION,
                       storageKey: "fathom.section.settings", defaultExpanded: false) {
            MonitorRow(label: "Version", value: UpdateChecker.displayLabel(FATHOM_VERSION), valueColor: .accent)
            ...
            Text(SUPPORT_DIR.path)
                ...
                .foregroundColor(.secondary)
                .lineLimit(2)
            Text("Local only — no accounts, no telemetry. Optional update check uses version.json only.")
                .font(uiFont(10))
                .foregroundColor(.secondary)
                .padding(.top, 4)
        }
...
struct MainRoot: View {
    var body: some View {
        VStack(spacing: 0) {
            // Header bar — monochrome menubar popover
            HStack(spacing: 10) {
                Text("Fathom")
                    .font(uiFont(16, weight: .semibold))
                    .foregroundColor(.primary)
            ...
            .padding(.vertical, 12)
            .background(Color.card.opacity(0.85))
            Divider().background(Color.border)
            ScrollView {
                VStack(spacing: 10) {
                    PowerSection()
        ...
        .frame(minWidth: 340, idealWidth: 380, maxWidth: 440, minHeight: 480, idealHeight: 560, maxHeight: 720)
        .background(Color.bg)
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private let popoverSize = NSSize(width: 380, height: 580)
