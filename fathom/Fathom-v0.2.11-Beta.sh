#!/bin/bash
# Fathom installer v0.2.11-Beta — BatFi sparkline + %/W + double-click
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="769f122052825780db0df4a3cbfffe12c62a9d07f17c0946bd6a52695d0c86fb"
...
let FATHOM_VERSION = "v0.2.11-Beta"
...
struct MainRoot: View {
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            ...
            // 1h package-watts sparkline (BatFi-style history)
            if power.wattHistory.filter({ $0 > 0.01 }).count >= 3 {
                VStack(alignment: .leading, spacing: 4) {
                    HStack {
                        Text("Package · last samples")
                    ...
                    Sparkline(values: power.wattHistory, color: .labelPrimary)
                        .frame(height: 32)
                        .padding(.vertical, 2)
...
// MARK: - Full dashboard window (literal product mock)

/// Non-menubar UI — pixel-close to the marketing card: faux menubar strip,
/// large %, package line, FATHOM pill, four KV rows. Nothing else.
struct DashboardRoot: View {
    ...
    private var packageSubtitle: String {
        // Exact mock wording: "On Battery · package 12.4 W"
    ...
    private var whyValue: String {
        // Mock shows short label: "Last hour"
    ...
    var body: some View {
        VStack(alignment: .leading, spacing: 0) {
            // ── Faux menubar chrome (matches mock top strip) ──
            HStack(spacing: 14) {
                Spacer(minLength: 0)
                Text("Wi‑Fi")
            ...
            // ── Hero: big % + package line + FATHOM pill ──
            ...
            // ── Four rows only (identical to mock) ──
            VStack(spacing: 0) {
                StatusKVRow(
                    label: "Power Source",
                    value: FathomStatusCopy.powerSourceLine(power: power)
                )
                StatusKVRow(
                    label: "Energy Mode",
                    value: FathomStatusCopy.energyModeLine(lowPower: lowPower)
                )
                StatusKVRow(
                    label: "Apps Using Energy",
                    value: topApp
                )
                StatusKVRow(
                    label: "Why drain?",
        ...
        .background(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
                .fill(Color(red: 0.09, green: 0.09, blue: 0.10)) // ~#17171A
        )
        .overlay(
            RoundedRectangle(cornerRadius: 18, style: .continuous)
...
enum MenuBarBatteryIcon {
    ...
    static func preferredLength(title: String, image: NSImage?) -> CGFloat {
        ...
        // Upper bound keeps room for "100%" / "12.4W" + bolt; lower bound for "0%"
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    /// Delay single-click popover so double-click can open dashboard without flashing the menu.
    private var pendingPopoverClick: DispatchWorkItem?
    ...
    @objc private func togglePopover() {
        ...
        // Double-click → dashboard; cancel pending single-click popover
        if event.clickCount >= 2 {
            pendingPopoverClick?.cancel()
            pendingPopoverClick = nil
        ...
        // Single-click: short delay so a second click can become a double-click
        pendingPopoverClick?.cancel()
        let work = DispatchWorkItem { [weak self] in
            self?.pendingPopoverClick = nil
            self?.performPopoverToggle()
        }
        pendingPopoverClick = work
    ...
    @objc func openDashboard() {
        ...
        // Borderless floating card — the SwiftUI view draws the rounded panel.
        let win = NSWindow(
        ...
        // Size to SwiftUI content after layout
        DispatchQueue.main.async {
            ...
            win.setContentSize(NSSize(width: max(size.width, 440), height: max(size.height, 320)))
            win.center()
        }
    }
