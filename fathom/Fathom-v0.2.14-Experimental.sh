#!/bin/bash
# Fathom installer v0.2.14-Beta — menubar time remaining
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="01c32025527eecbb326927d653339f59e1e62e596dcb1d2d34949a40911bd6cf"
...
let FATHOM_VERSION = "v0.2.14-Beta"
                ...
                VStack(alignment: .leading, spacing: 0) {
                    ...
                    if showMore {
                        VStack(alignment: .leading, spacing: 8) {
                            ...
                            .labelsHidden()
                            Text("Time = how long left (or to full while charging). W = package watts.")
                                .font(uiFont(10))
                                .foregroundColor(.labelTertiary)
                                .fixedSize(horizontal: false, vertical: true)
...
/// Customize sheet / side panel for dashboard.
...
// MARK: - Full dashboard window (customizable non-menubar UI)
...
enum MenuBarBatteryIcon {
    ...
    /// Label next to battery icon: time remaining by default; watts mode still available.
    static func menubarLabel(
        ...
        // Default (percent mode): show time left next to the battery icon
        ...
        // Fallback while estimate is still calculating
    ...
    static func image(
        ...
        // Slightly smaller font for longer "2h 15m" strings
        let fontSize: CGFloat = label.count > 5 ? 11 : 12
    ...
    /// Status-item width from composite image (room for "12h 59m" + bolt glyph).
    static func preferredLength(title: String, image: NSImage?) -> CGFloat {
        _ = title
        let w = (image?.size.width ?? 40) + 8
        return min(max(w.rounded(.up), 36), 130)
    }
}
...
final class AppDelegate: NSObject, NSApplicationDelegate, NSPopoverDelegate {
    ...
    private func updateStatusTitle() {
        ...
        if p.batteryPresent {
            button.toolTip = "Fathom · \(p.levelPercent)% · \(p.timeText) · \(String(format: "%.1f W", p.packageWatts))"
        } else {
            button.toolTip = "Fathom · AC"
        }
    }
    ...
    private func resizeDashboardWindow() {
        ...
        DispatchQueue.main.async {
            ...
            // When customize panel is open, width expands ~280pt
...
<key>CFBundleVersion</key><string>0.2.14</string>
