#!/bin/bash
# Fathom installer v0.2.18-Beta — more accurate sensors
set -Eeuo pipefail
IFS=$'\n\t'
umask 077
...
EXPECTED_HASH="65fe09d4599a10bbbdbd8c617c59988c92e0d9117cec05d89a7313f0cb9cd7ee"
...
let FATHOM_VERSION = "v0.2.18-Beta"
...
/// Package watts history length (1h @ 2s poll).
...
final class PowerStore: ObservableObject {
    ...
    /// When the menubar popover is open, poll faster for a snappier UI.
    private var fastMode = false

    func start() {
        restartTimer(interval: fastMode ? 0.9 : 1.25)
        poll()
    }
    ...
    func setFastMode(_ on: Bool) {
        ...
        fastMode = on
        restartTimer(interval: on ? 0.9 : 1.25)
        if on { poll() }
    }
    ...
    private func poll() {
        DispatchQueue.global(qos: .userInitiated).async { [weak self] in
            // Battery + IOReport on background; dual IOReport sample improves first-window accuracy
            let bat = Self.readBattery()
            var pkg = Self.readPackageWattsDetailed()
            // Second sample shortly after first if first was warmup (0 W / estimate)
            if pkg.source == "estimate" || pkg.watts < 0.1 {
                Thread.sleep(forTimeInterval: 0.35)
                let pkg2 = Self.readPackageWattsDetailed()
                if pkg2.watts > pkg.watts { pkg = pkg2 }
            }
            ...
            DispatchQueue.main.async {
                ...
                if pkg.watts > 0 {
                    // Adaptive EMA: faster when power jumps hard
    ...
    static func readBattery() -> BatSnap {
        var snap = BatSnap()
        // Prefer IOPowerSources for menu-bar % (matches system Battery %)
        if let info = IOPSCopyPowerSourcesInfo()?.takeRetainedValue(),
           let list = IOPSCopyPowerSourcesList(info)?.takeRetainedValue() as? [CFTypeRef] {
            for src in list {
                ...
                snap.present = true
                // Capacity: IOPS usually reports percent 0–100
                ...
                if let cap, let maxC, maxC > 0, maxC != 100, cap > 100 {
                    snap.level = min(100, Int((Double(cap) / Double(maxC) * 100).rounded()))
                } else if let cap, cap >= 0, cap <= 100 {
                    snap.level = cap
                }
                let state = d[kIOPSPowerSourceStateKey] as? String ?? ""
                snap.onAC = state == kIOPSACPowerValue || state == "AC Power"
                if let ch = d[kIOPSIsChargingKey] as? Bool {
                    snap.charging = ch
                } else if let n = d[kIOPSIsChargingKey] as? NSNumber {
                    snap.charging = n.boolValue
                }
                // Time to empty / full (IOPS uses -1 while calculating)
                func iopsInt(_ key: String) -> Int? {
                    if let t = d[key] as? Int { return t }
                    if let n = d[key] as? NSNumber { return n.intValue }
                    return nil
                }
                if let t = iopsInt(kIOPSTimeToEmptyKey), t > 0, t < 65535, !snap.charging {
                    snap.minutes = t
                }
                if let t = iopsInt(kIOPSTimeToFullChargeKey), t > 0, t < 65535, snap.charging {
                    snap.minutes = t
                }
                break
            }
        ...
        // Enrich via AppleSmartBattery (raw mAh, InstantAmperage, adapter)
        let service = IOServiceGetMatchingService(0, IOServiceMatching("AppleSmartBattery"))
        if service == 0 { return finalizeBatterySnap(snap) }
        ...
        snap.rawCurrent = rawCur
        snap.rawMax = rawMax

        // Prefer chemical SoC from raw mAh when available (more stable than rounded IOPS %)
        if let raw = rawCur, let mx = rawMax, mx > 0 {
            let chem = min(100, max(0, Int((Double(raw) / Double(mx) * 100).rounded())))
            // Blend: if IOPS % missing or differs a lot, trust raw
            if snap.level <= 0 || abs(chem - snap.level) >= 2 {
                snap.level = chem
            }
        } else if snap.level <= 0 {
        ...
        if let fully = propBool(service, "FullyCharged"), fully {
            snap.charging = false
            snap.onAC = true
            if snap.level < 100 { snap.level = max(snap.level, 99) }
        }
        ...
        if let rawMax, design > 0 {
            snap.health = min(120, max(0, Int((Double(rawMax) / Double(design) * 100).rounded())))
            if let h = snap.health { snap.health = min(100, h) }
        }

        // Voltage (mV) — BatteryData fallback like rNitro
        ...
        if let amp = propInt(service, "InstantAmperage") ?? propInt(service, "Amperage") {
            ...
            if signed != 0, voltage > 0 {
                ...
                else if abs(signed) > 20 { snap.chargeW = w } // rare sign quirks
            }
        }

        // Adapter watts when charging
        if let adapter = prop(service, "AdapterDetails") as? [String: Any] {
            if let w = (adapter["Watts"] as? NSNumber)?.doubleValue, w > 0 {
                snap.adapterW = w
            } else if let v = (adapter["AdapterVoltage"] as? NSNumber)?.doubleValue,
                      let c = (adapter["Current"] as? NSNumber)?.doubleValue, v > 0, c > 0 {
                // often mV / mA
                snap.adapterW = (v > 100 ? v / 1000.0 : v) * (c > 50 ? c / 1000.0 : c)
            }
        }
        if snap.charging, snap.chargeW <= 0.1, snap.adapterW > 0 {
            // Adapter rating as upper-bound proxy when instant amps unavailable
            snap.chargeW = min(snap.adapterW, 140)
        }
        ...
        // Time remaining from SMC averages
        if snap.minutes == nil {
            if snap.charging,
        ...
        // Live pack-draw estimate when system still “Calculating…”
...
struct DashboardRoot: View {
    ...
    private var powerSection: some View {
        MonitorSection(
            ...
            MonitorRow(label: "Time left", value: FathomCopy.friendlyTime(power: power), valueColor: .labelSecondary)
            if prefs.showHealth {
                if let h = power.healthPercent {
                    MonitorRow(label: "Health", value: "\(h)%", valueColor: .labelPrimary)
                }
